...
export default defineComponent({
  ...
  render() {
    const { name, followers, following, email, location, company, avatar_url } = this.dataSource
    ...
    return (
      <div class={Styles['base-info-container']}>
        <img class={Styles.avatar} src={avatar_url}></img>
        <div class={Styles['user-name']}>{name}</div>
        <div class={Styles.followers}>
          <svg
            ...
            version="1.1"
            width="16"
            class={Styles['svg-icon']}
            data-view-component="true">
            <path fill-rule="evenodd" d={MapSvgInfo.user}></path>
          </svg>
          <div class={Styles['follower-info']}>
            <span class={Styles.txt}> {followers}</span> followers ·<span class={Styles.txt}> {following}</span>{' '}
            following
          </div>
        </div>
